set SAN=fake-cert-to-delete-mycompany.com 
set DN="CN=%SAN%,OU=IT,O=My Company,L=Paris,ST=IDF,C=FR,emailAddress=no-reply@groland.grd"

keytool -genkeypair -alias fake-cert-to-delete -keystore .\tmp\empty-TrustStore.jks -dname %DN% -ext SAN=dns:%SAN% -storepass %SYS_STOREPASS% -keypass %SYS_STOREPASS% -storetype %STORE_TYPE% -keyalg EC -keysize 256 -keyalg RSA -keysize 2048 -validity 1  -v 
keytool -delete -alias fake-cert-to-delete -keystore .\tmp\empty-TrustStore.jks -storepass %SYS_STOREPASS%

copy .\tmp\empty-TrustStore.jks %SYS_TRUSTED_CERT%
copy .\tmp\empty-TrustStore.jks %SYS_TRUSTED_SITE_CERT%
copy .\tmp\empty-TrustStore.jks %SYS_CLIENT_AUTH_CERT%

REM  Default keystore type is set in property keystore.type=jks at  C:\Program Files (x86)\Java\jre1.8.0_131\lib\security\java.security & keystore.type=pkcs12 in C:\Program Files\Java\jdk-9.0.1\conf\security\java.security
