set SAN=fake-cert-to-delete-pprd.apps.airliquide.com 
set DN="CN=%SAN%,OU=IT,O=Company,L=Paris,ST=IDF,C=FR,emailAddress=no-reply@company.com"

keytool -genkeypair -alias fake-cert-to-delete -keystore .\tmp\empty-TrustStore.p12 -dname %DN% -ext SAN=dns:%SAN% -storepass %SYS_STOREPASS% -storetype %STORE_TYPE% -keyalg EC -keysize 256 -validity 1  -v 
keytool -delete -alias fake-cert-to-delete -keystore .\tmp\empty-TrustStore.p12 -storepass %SYS_STOREPASS%
